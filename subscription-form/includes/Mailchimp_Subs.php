    ...
    public function widget( $args, $instance ) {
        ...
        echo $before_widget;
            if( ! empty( $title ) ) {
                ...
                echo __( 'Demo Widget Title', 'weSubs' );
            }
?>
        <div id="mailchimp_signup">
            <form method="post">
				<p>
					<label>
						<input style="width: 100%;" type="email" name="email" placeholder="Enter email address" required />
					</label>
				</p>
			...
			<?php
				if ( wp_remote_retrieve_response_code( $response ) == 200 ) {
					foreach ( $body->lists as $list ) {
						...
						<option value="<?php echo $form_id; ?>" <?php selected( $subscriber_list, $form_id ); ?>>
							<?php echo $form_name; ?>
						</option>
